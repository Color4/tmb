...
# combine exome/tcga muts (1d5% threshold)
...
master_mut_sum <- master_mut %>%
  group_by(bcr_patient_barcode,panel_size,vfthresh) %>%
  summarise(wes_count = mean(wes_mut_count),
            wes_rate = mean(wes_mut_mb),
            panel_count = mean(panel_muts),
            panel_rate = mean(panel_muts_mb),
            panel_rate_sd=sd(panel_muts_mb)) %>%
  # filter out high sd in panel_rate calculations
  filter(panel_rate_sd < 5)

# grab full variant set data ('vfthresh == 0')
vf0df <- master_mut_sum %>%
  filter(vfthresh == 0) %>%
  ungroup() %>%
  select(bcr_patient_barcode,"wes_count_truth"=wes_count,"wes_rate_truth"=wes_rate) %>%
  unique() %>%
  mutate(rate_group = cut(wes_rate_truth,c(0,10,20,40,100,10000)))

# merge 'truth' in with sampled data
vf0df %>%
...
ggplot(master_mut_sum2[master_mut_sum2$panel_size < 5,],aes(panel_size,rsq,color=vfthresh,group=vfthresh)) + 
  ...
  ggtitle("Correlation w/'Truth' WES Mut Rate")
...
master_mut_sum3 %>%
  filter(wes_rate_truth >= 10 & wes_rate_truth <= 100 & panel_size %in% c(0.5,1.5,2,3,4,5)) %>%
...
master_mut_sum3 %>%
  filter(wes_rate_truth >= 10 & wes_rate_truth <= 100 & panel_size %in% c(5) & vfthresh == 0) -> tmp3

lmtest <- lm(formula=tmp3$wes_rate_truth~tmp3$panel_rate,df=df)
t <- resid(lmtest)
...
b <- mydf[mydf$resid < -15,c("bcr_patient_barcode")]
